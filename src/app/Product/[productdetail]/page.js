...
import { addToCart } from "@/app/redux/addToCart"
import { useRouter } from "next/navigation"

const ProductDetail = ({params}) => {

  let[singlePostData , updateSinglePostData ] = useState({})
  ...
  useEffect(() => {
    ...
    async function Detailing(){
      let daata = await fetch(`http://localhost:4000/products/${params.productdetail}`)
      let newData = await daata.json()
      updateSinglePostData(newData)
    }
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="lg:col-gap-12 xl:col-gap-16 mt-8 grid grid-cols-1 gap-12 lg:mt-12 lg:grid-cols-5 lg:gap-16">
                ...
                <div className="lg:flex lg:items-start">
                  <div className="lg:order-2 lg:ml-5">
                    <div className="max-w-xl overflow-hidden rounded-lg">
                      <img className="h-full w-full max-w-full object-cover" src={singlePostData.thumbnail} alt="" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2">
                <h1 className="sm: text-2xl font-bold text-gray-900 sm:text-3xl">{singlePostData.title}</h1>
                ...
                <div className="mt-3 flex select-none flex-wrap items-center gap-1">
                  <label className="">
                    ...
                    <span className="mt-1 block text-center text-xs">2000/mo</span>
                  </label>
                  <label className="">
                    ...
                    <span className="mt-1 block text-center text-xs">1600/mo</span>
                  </label>
                  <label className="">
                    ...
                    <span className="mt-1 block text-center text-xs">1200/mo</span>
                  </label>
                </div>

                <div className="mt-10 flex flex-col items-center justify-between space-y-4 border-t border-b py-4 sm:flex-row sm:space-y-0">
                  <div className="flex items-end">
                    <h1 className="text-3xl font-bold">Rs {Math.floor(singlePostData.price * ( 100 - singlePostData.discountPercentage) /  100 ) * 80}</h1>
                  </div>

                  <button type="button" className="bg-slate-900 text-white px-12 py-3 rounded-md border-3 border-transparent inline-flex items-center justify-center" onClick={() =>sendCartData()} >
