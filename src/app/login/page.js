...
import { sendLoginCredintials } from "../redux/sendProfileCredentials";
...
export default function Login() {
    ...
    let [userCred , updateUserCred] = useState({
        emailAddress:"",
        password:""
    })

    let [checkUserCred , updateUser] = useState([])

    useEffect(() => {checkUser()},[])

    async function checkUser (){
        let getData = await fetch("http://localhost:4003/users")
        let setData = await getData.json()
        updateUser(setData)
    }


    const updateUserCredentials = (e) => {
        updateUserCred(prevstate => ({...prevstate,[e.target.name]:e.target.value}))
    }


    const submitCredDetails = (e) => {
        e.preventDefault();
       let checkingUserCred = checkUserCred.find((item) => item.emailAddress === userCred.emailAddress)
       ...
       if(checkingUserCred){
        ...
        dispatch(sendLoginCredintials(checkingUserCred))
    }
  ...
  return (
    <>
        ...
        <form class="w-full max-w-md" onSubmit={submitCredDetails} >
            ...
            <div class="relative flex items-center mt-8">
                ...
                <input type="email" class="block w-full py-3 text-gray-700 bg-white border rounded-lg px-11 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" placeholder="Email address" name="emailAddress" onChange={updateUserCredentials} />
            </div>
            ...
            <div class="relative flex items-center mt-4">
                ...
                name="password" onChange={updateUserCredentials}/>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="mt-6 text-center ">
                    <Link href="/register" class="text-sm text-blue-500 hover:underline dark:text-blue-400">
